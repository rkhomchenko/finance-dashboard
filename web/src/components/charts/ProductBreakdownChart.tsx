  ...
  const { chartData, productNames } = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      if (!monthsMap.has(month)) {
        monthsMap.set(month, { label: month } as Record<string, number>);
      }
    ...
    const chartData = Array.from(monthsMap.values()).sort((a, b) => {
      const labelA = a.label as unknown as string;
      const labelB = b.label as unknown as string;
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 h-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-medium text-gray-900">Revenue by Product</h3>
        ...
        <LineChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
          ...
          {selectedProductNames.map((name, idx) => (
            <Line
              ...
              dataKey={name}
              name={name}
              stroke={PRODUCT_COLORS[name] || DEFAULT_COLORS[idx % DEFAULT_COLORS.length]}
              strokeWidth={2}
              dot={{ r: 3, fill: PRODUCT_COLORS[name] || DEFAULT_COLORS[idx % DEFAULT_COLORS.length] }}
              activeDot={{ r: 5 }}
            />
          ))}
        </LineChart>
